...
namespace Hackaton.API.Controllers
{
    ...
    public class ParticipantsController : ControllerBase
    {
        ...
        {
            var Participant = await _context.Participants.FirstOrDefaultAsync(x => x.ID_Participant==id);
            if (Participant == null)
            {
                return NotFound();
            }
        ...
        {
            var FilasAfectadas = await _context.Participants.Where(a => a.ID_Participant==id).ExecuteDeleteAsync();
            if (FilasAfectadas == 0)
            {
                return NotFound();
            }
