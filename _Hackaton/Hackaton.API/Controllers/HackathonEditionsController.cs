...
namespace Hackaton.API.Controllers
{
    ...
    public class HackathonEditionsController : ControllerBase
    {
        ...
        {
            var HackathonEdition = await _context.HackathonEditions.FirstOrDefaultAsync(x => x.HackathonId==id);
        ...
        {
            var FilasAfectadas = await _context.HackathonEditions.Where(a => a.HackathonId==id).ExecuteDeleteAsync();
            if (FilasAfectadas == 0)
            {
                return NotFound();
            }
